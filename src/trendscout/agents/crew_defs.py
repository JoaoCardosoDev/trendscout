...
def create_trend_to_post_crew():
    ...
    return trend_to_post_crew, trend_analysis_task, content_generation_task, scheduling_task
...
def run_trend_to_post_workflow(topic: str) -> dict:
    ...
    try:
        crew, trend_analysis_task_obj, content_generation_task_obj, scheduling_task_obj = create_trend_to_post_crew()
        ...
        for agent_name, task_obj in tasks_with_names:
            ...
            if task_obj and task_obj.output:
                if hasattr(task_obj.output, 'exported_output') and task_obj.output.exported_output:
                    step_output = task_obj.output.exported_output
                elif hasattr(task_obj.output, 'raw_output') and task_obj.output.raw_output:
        ...
        logger.error(f"Error during Trend-to-Post workflow for topic '{topic}': {e}", exc_info=True)
