...
def process_task(task_data: dict):
    ...
    try:
        db_task = (
            db.query(AgentTask).filter(AgentTask.task_id == task_id_from_queue).first()
        )
        if not db_task:
        ...
        if agent_type == "trend_analyzer":
            ...
            logger.info(
                f"Running TrendAnalyzerAgent for task {task_id_from_queue} with query: '{query}'"
            )
            ...
            query = input_data.get("query", "")
            logger.info(
                f"Running ContentGeneratorAgent for task {task_id_from_queue} with query: '{query}'"
            )
            result_obj = agent.run(query)
            ...
            query = input_data.get("query", "")
            logger.info(
                f"Running SchedulerAgent for task {task_id_from_queue} with query: '{query}'"
            )
            result_obj = agent.run(query)
            ...
            if topic:
                logger.info(
                    f"Running Trend-to-Post Crew for task {task_id_from_queue} with topic: '{topic}'"
                )
                result_obj = run_trend_to_post_workflow(
                    topic
                )  # This returns CrewOutput
            else:
            ...
            if isinstance(result_obj, dict) and result_obj.get("error"):
                ...
                if agent_type == "trend_to_post_crew":
                    ...
                    if hasattr(result_obj, "raw") and result_obj.raw is not None:
                        ...
                        hasattr(result_obj, "tasks_output")
                        and result_obj.tasks_output
                        and hasattr(result_obj.tasks_output[-1], "raw_output")
                    ):
                        serializable_result["final_output"] = result_obj.tasks_output[
                            -1
                        ].raw_output
                    elif (
                        hasattr(result_obj, "json_output")
                        and result_obj.json_output is not None
                    ):  # CrewAI sometimes uses json_output
                        serializable_result["final_output"] = result_obj.json_output
                    elif (
                        hasattr(result_obj, "pydantic_output")
                        and result_obj.pydantic_output is not None
                    ):  # Or pydantic_output
                        try:
                            serializable_result["final_output"] = (
                                result_obj.pydantic_output.model_dump_json()
                            )
                        except (
                            AttributeError
                        ):  # If not a pydantic model, convert to string
                            serializable_result["final_output"] = str(
                                result_obj.pydantic_output
                            )
                    else:
                    ...
                    if hasattr(result_obj, "tasks_output") and result_obj.tasks_output:
                        for task_output_item in result_obj.tasks_output:
                            ...
                            if (
                                hasattr(task_output_item, "agent")
                                and task_output_item.agent
                            ):
                                agent_name_str = str(task_output_item.agent)
                            elif (
                                hasattr(task_output_item, "task")
                                and task_output_item.task
                                and hasattr(task_output_item.task, "agent")
                                and task_output_item.task.agent
                                and hasattr(task_output_item.task.agent, "role")
                            ):
                                agent_name_str = task_output_item.task.agent.role

                            step_output = (
                                task_output_item.raw_output
                                if hasattr(task_output_item, "raw_output")
                                else str(task_output_item)
                            )
                            ...
                            if hasattr(task_output_item, "timestamp"):
                                if isinstance(task_output_item.timestamp, datetime):
                                    timestamp_str = (
                                        task_output_item.timestamp.isoformat()
                                    )
                                else:
                    ...
                    if (
                        hasattr(result_obj, "usage_metrics")
                        and result_obj.usage_metrics
                    ):
                        serializable_result["usage_metrics"] = result_obj.usage_metrics

                    db_task.result = serializable_result
                    db_task.intermediate_steps = (
                        serializable_intermediate_steps
                        if serializable_intermediate_steps
                        else None
                    )

                else:  # For single agents
                    if (
                        isinstance(result_obj, (dict, list, str, int, float, bool))
                        or result_obj is None
                    ):
                        db_task.result = result_obj
                    else:
                        try:
                            ...
                            Exception
                        ):  # Catch any error during serialization attempt
                            db_task.result = {"output": str(result_obj)}
...
def main_worker_loop():
    ...
    while True:
        ...
        for queue_name in queues_to_monitor:
            ...
            task_data_json = queue_manager.redis_client.rpop(
                f"queue:{queue_name}"
            )  # Direct Redis call for simplicity here

            if task_data_json:
                try:
                    task_data = json.loads(task_data_json)
                    logger.info(
                        f"Dequeued task from {queue_name}: {task_data.get('task_id')}"
                    )
                    ...
                    logger.error(
                        f"Failed to decode task data from {queue_name}: {task_data_json}"
                    )
                except Exception as e:
                    logger.error(
                        f"Unexpected error processing task from {queue_name}: {e}",
                        exc_info=True,
                    )
            # else:
        ...
        if not processed_a_task_this_cycle:
            ...
            time.sleep(
                settings.OLLAMA_TIMEOUT / 100 if settings.OLLAMA_TIMEOUT > 100 else 5
            )  # Sleep if no tasks found, e.g., 5 seconds
